...
func WebRPCSchemaHash() string {
	return "2ca934c1978c6c011463eacb1ca3f263bd7b0591"
}

type EmailService interface {
	SendEmail(ctx context.Context, email string, name string, body string, price uint64, electricity float32, gas float32, token string) (bool, error)
}
...
func NewEmailServiceServer(svc EmailService) WebRPCServer {
	return &emailServiceServer{
		EmailService: svc,
	}
}

func (s *emailServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	ctx = context.WithValue(ctx, HTTPResponseWriterCtxKey, w)
	ctx = context.WithValue(ctx, HTTPRequestCtxKey, r)
	ctx = context.WithValue(ctx, ServiceNameCtxKey, "EmailService")
	...
	switch r.URL.Path {
	case "/rpc/EmailService/SendEmail":
		s.serveSendEmail(ctx, w, r)
		return
	default:
...
func (s *emailServiceServer) serveSendEmail(ctx context.Context, w http.ResponseWriter, r *http.Request) {
...
func (s *emailServiceServer) serveSendEmailJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "SendEmail")
	reqContent := struct {
		Arg0 string  `json:"email"`
		Arg1 string  `json:"name"`
		Arg2 string  `json:"body"`
		Arg3 uint64  `json:"price"`
		Arg4 float32 `json:"electricity"`
		Arg5 float32 `json:"gas"`
		Arg6 string  `json:"token"`
	}{}
	...
	func() {
		...
		ret0, err = s.EmailService.SendEmail(ctx, reqContent.Arg0, reqContent.Arg1, reqContent.Arg2, reqContent.Arg3, reqContent.Arg4, reqContent.Arg5, reqContent.Arg6)
	}()
	respContent := struct {
